  ...
  import CreateIncidentModal, {type CreateIncidentPayload,} from "./CreateIncidentModal";
  import { useGetIncidentsQuery, useCreateIncidentMutation, useUpdateIncidentMutation, useDeleteIncidentMutation, } from "@/feature/RoomAssetApi/incident.api";
  import EditIncidentModal, { type EditIncidentPayload } from "./EditIncidentModal";
  ...
  type RoomRowWithName = RoomRow & { name?: string; id: string; building_id?: string };
  ...
  export default function RoomDetails({ room, onBack, buildings, onUpdated }: Props) {
    ...
    const [createIncident, { isLoading: isCreatingIncident }] = useCreateIncidentMutation();
    const [updateIncident] = useUpdateIncidentMutation();
    const [deleteIncident, { isLoading: isDeletingIncident }] = useDeleteIncidentMutation();
            ...
            <Box sx={{ display: "flex", gap: 0.5, justifyContent: "center" }}>
              ...
              <Tooltip title="Xóa">
                <IconButton
                  ...
                  onClick={async () => {
                    const ok = window.confirm("Bạn có chắc chắn muốn xóa sự việc này?");
                    if (!ok) return;
        ...
        <div className="mt-10 rounded-2xl border border-gray-100 bg-white shadow-sm overflow-hidden">
          <div className="px-5 py-4 bg-[#f8f9fa] border-b border-gray-100 flex items-center justify-between">
            ...
            <CreateIncidentModal
              ...
              facilities={facilitiesFromApi.map((f) => ({ id: f.id, name: f.name }))}
              onCreated={async (payload: CreateIncidentPayload) => {
                try {
                  const userStr = localStorage.getItem("user");
                  const user = userStr ? JSON.parse(userStr) : null;
                  const createdBy = user?.id || user?.userId || "";
                  console.log("Creating incident with payload:", payload, "by user:", createdBy);
                  console.log("Mapped status to API value:", mapIncidentStatusToApi(payload.status));
